...
# Pfad zur ffmpeg.exe (übergeordnetes Verzeichnis)
FFMPEG_PATH = os.path.join(os.path.dirname(os.getcwd()), 'ffmpeg.exe')
...
def process_streamer(streamer_name):
    ...
    for video in videos_data['data']:
        ...
        # Video herunterladen mit yt-dlp und ffmpeg verwenden
        ...
        try:
            ...
            # Optional: ffmpeg für zusätzliche Verarbeitung (z.B. Audio extrahieren oder umwandeln)
            subprocess.run([FFMPEG_PATH, "-i", output_path, "-vn", "-acodec", "libmp3lame", output_path.replace(".mp4", ".mp3")], check=True)
            print(f"Audio erfolgreich extrahiert und gespeichert.")
            
            downloaded_count += 1
        except subprocess.CalledProcessError as e:
            print(f"Fehler beim Herunterladen des Videos oder Verarbeiten des Audio: {e}")
