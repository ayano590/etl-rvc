...
input_audio0 = lv_orig_dir + "\\audio"  # just the path to the tw original wav folder, adding video titles in the code
...
def lv_infer():
    # infer all lv tracks with all trained models
    for file_name in os.listdir(input_audio0):
        [title, file_format] = file_name.split(".")

        # set track base level
        if title in male_voice:
            track_voice = 0
        else:
            track_voice = 12
        ...
        for lang in os.listdir(weights_dir):
            ...
            # transpose
            vc_transform0 = infer_voice - track_voice
            ...
            output_path = f"{lv_conv_dir}\\audio\\{title}_{spk_lang}.wav"
